...
#pragma region K-NN search
        ...
        ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
        {
            ...
            // Use per-query search budget (nprobe) instead of fixed m_searchInternalResultNum
            int querySearchBudget = p_query.GetSearchBudget();
            if (p_query.GetResultNum() >= querySearchBudget)
                p_queryResults = (COMMON::QueryResultSet<T> *)&p_query;
            else
                p_queryResults = new COMMON::QueryResultSet<T>((const T *)p_query.GetTarget(), querySearchBudget, p_query.GetQueryID());
            ...
            if (m_extraSearcher != nullptr)
            {
                ...
                for (int i = 0; i < querySearchBudget; ++i)
                {
                    ...
                    if (workSpace->m_postingIDs.size() >= querySearchBudget ||
        ...
        ErrorCode Index<T>::SearchIndex(std::vector<QueryResult> &queries, bool p_searchDeleted) const
        {
            ...
            for (int query = 0; query < queries.size(); query++)
            {
                ...
                if (p_query.GetResultNum() >= querySearchBudget)
                    ...
                    vecQueryResultSet.push_back(new COMMON::QueryResultSet<T>((const T *)p_query.GetTarget(), querySearchBudget, p_query.GetQueryID()));
            ...
            for (int i = 0; i < vecQueryResultSet.size(); i++)
            {
                ...
                int querySearchBudget = queries[i].GetSearchBudget();
                if (queries[i].GetResultNum() < querySearchBudget)
                {
                ...
                {
                    std::copy(query_set->GetResults(), query_set->GetResults() + querySearchBudget, queries[i].GetResults());
                }
            }
...
#pragma endregion
        ...
        ErrorCode Index<T>::BuildIndexInternal(std::shared_ptr<Helper::VectorSetReader> &p_reader)
        {
            ...
            if (m_options.m_enableSSD)
            {
                ...
                // FIX: Restore DistCalcMethod after loading head index from disk
                m_index->SetParameter("DistCalcMethod", SPTAG::Helper::Convert::ConvertToString(m_options.m_distCalcMethod));
