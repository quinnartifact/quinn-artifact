...
public:
    SearcherOptions() : Helper::ReaderOptions(VectorValueType::Float, 0, VectorFileType::TXT, "|", 32)
    {
        ...
        AddOptionalOption(m_latencyShm, "-lshm", "--latency_shm", "QUINN: POSIX SHM name for per-query latency timestamps.");
    }
...
int Process(std::shared_ptr<SearcherOptions> options, VectorIndex& index)
{
    ...
    // QUINN: open latency SHM now that total query count is known.
    if (!options->m_latencyShm.empty()) {
        try {
            latencyShm = std::make_unique<quinn::LatencyShmAccessor>(
                options->m_latencyShm, static_cast<uint32_t>(queryVectors->Count()));
        } catch (const std::exception &e) {
            LOG(Helper::LogLevel::LL_Warning, "LatencyShmAccessor init failed: %s\n", e.what());
        }
    }
        ...
        for (int mc = 0; mc < maxCheck.size(); mc++)
        {
            ...
            auto func = [&]()
            {
                ...
                while (true)
                {
                    ...
                    if (qid < numQuerys)
                    {
                        int64_t t1_ns = quinn::LatencyShmAccessor::now_ns();
                        auto t1 = std::chrono::high_resolution_clock::now();
                        index.SearchIndex(results[qid]);
                        auto t2 = std::chrono::high_resolution_clock::now();
                        int64_t t2_ns = quinn::LatencyShmAccessor::now_ns();
                        latencies[qid] = (float)(std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() / 1000000.0);
                        if (latencyShm) {
                            uint32_t global_qid = static_cast<uint32_t>(startQuery + qid);
                            latencyShm->record_spann_start(global_qid, t1_ns);
                            latencyShm->record_spann_end  (global_qid, t2_ns);
                        }
                    }
                    else
                    {
