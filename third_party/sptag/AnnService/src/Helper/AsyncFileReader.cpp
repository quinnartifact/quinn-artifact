...
namespace SPTAG {
    namespace Helper {
        ...
        void BatchReadFileAsync(std::vector<std::shared_ptr<Helper::DiskIO>>& handlers, AsyncReadRequest* readRequests, int num)
        {
            ...
            if (first_call) {
                fprintf(stderr, "[SPANN] BatchReadFileAsync called with %d requests\n", num);
                first_call = false;
            }
            ...
            // io_trace: capture complete timestamp and record events
            {
                ...
                std::vector<uint64_t> _t_offsets(num), _t_sizes(num);
                for (int i = 0; i < num; i++) {
                    _t_offsets[i] = readRequests[i].m_offset;
                    _t_sizes[i]   = readRequests[i].m_readSize;
                }
                io_trace::record_batch("io_read",
                    _trace_submit_ts, _trace_complete_ts,
                    _t_offsets.data(), _t_sizes.data(), (uint64_t)num);
            }
        }
#else
