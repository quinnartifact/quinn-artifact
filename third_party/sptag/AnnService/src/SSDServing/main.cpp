...
namespace SPTAG {
	namespace SSDServing {
		...
		int BootProgram(bool forANNIndexTestTool,
			...
			const char* budgetShmName,
			const char* earlyExitShmName,
			int earlyExitKRef,
			const char* threadCountShmName,
			const char* latencyShmName) {
			...
			if (forANNIndexTestTool) {
				...
				// QUINN: Set BudgetShmName AFTER loading config to avoid being overwritten
				if (budgetShmName != nullptr && std::string(budgetShmName).length() > 0) {
					(*config_map)[SEC_BUILD_SSD_INDEX]["BudgetShmName"] = budgetShmName;
					LOG(Helper::LogLevel::LL_Info, "QUINN: Setting BudgetShmName to %s\n", budgetShmName);
				}
				if (earlyExitShmName != nullptr && std::string(earlyExitShmName).length() > 0) {
					(*config_map)[SEC_BUILD_SSD_INDEX]["EarlyExitShm"] = earlyExitShmName;
					LOG(Helper::LogLevel::LL_Info, "QUINN: Setting EarlyExitShm to %s\n", earlyExitShmName);
				}
				if (earlyExitKRef > 0) {
					(*config_map)[SEC_BUILD_SSD_INDEX]["EarlyExitKRef"] = std::to_string(earlyExitKRef);
					LOG(Helper::LogLevel::LL_Info, "QUINN: Setting EarlyExitKRef to %d\n", earlyExitKRef);
				}
				if (threadCountShmName != nullptr && std::string(threadCountShmName).length() > 0) {
					(*config_map)[SEC_BUILD_SSD_INDEX]["ThreadCountShmName"] = threadCountShmName;
					LOG(Helper::LogLevel::LL_Info, "QUINN: Setting ThreadCountShmName to %s\n", threadCountShmName);
				}
				if (latencyShmName != nullptr && std::string(latencyShmName).length() > 0) {
					(*config_map)[SEC_BUILD_SSD_INDEX]["LatencyShmName"] = latencyShmName;
					LOG(Helper::LogLevel::LL_Info, "QUINN: Setting LatencyShmName to %s\n", latencyShmName);
				}
			}
...
int main(int argc, char* argv[]) {
	if (argc < 2)
	{
		LOG(Helper::LogLevel::LL_Error,
			"ssdserving configFilePath [--budget_shm <shm_name>] [--io_trace <output.json>]\n");
		exit(-1);
	}
	...
	std::string early_exit_shm_name;
	std::string thread_count_shm_name;
	std::string latency_shm_name;
	int k_ref = 100; // Default k_ref
	for (int i = 2; i < argc; i++) {
		...
		else if (std::string(argv[i]) == "--io_trace" && i + 1 < argc) {
			io_trace_path = argv[i + 1];
			LOG(Helper::LogLevel::LL_Info, "I/O trace output: %s\n", io_trace_path.c_str());
			i++;
		}
		else if (std::string(argv[i]) == "--early_exit_shm" && i + 1 < argc) {
			early_exit_shm_name = argv[i + 1];
			LOG(Helper::LogLevel::LL_Info, "Using early exit shared memory: %s\n", early_exit_shm_name.c_str());
			i++;
		}
		else if (std::string(argv[i]) == "--k_ref" && i + 1 < argc) {
			k_ref = std::stoi(argv[i + 1]);
			LOG(Helper::LogLevel::LL_Info, "Using k_ref: %d\n", k_ref);
			i++;
		}
		else if (std::string(argv[i]) == "--thread_count_shm" && i + 1 < argc) {
			thread_count_shm_name = argv[++i];
			LOG(Helper::LogLevel::LL_Info, "Dynamic thread count SHM: %s\n", thread_count_shm_name.c_str());
		}
		else if (std::string(argv[i]) == "--latency_shm" && i + 1 < argc) {
			latency_shm_name = argv[++i];
			LOG(Helper::LogLevel::LL_Info, "Latency SHM: %s\n", latency_shm_name.c_str());
		}
	}

	// QUINN: Initialize I/O tracing if --io_trace is provided
	io_trace::init("spann", io_trace_path);

	std::map<std::string, std::map<std::string, std::string>> my_map;

	// Pass budget_shm_name to BootProgram to be set AFTER loading config
	auto ret = SSDServing::BootProgram(false, &my_map, argv[1],
		VectorValueType::Undefined, DistCalcMethod::Undefined,
		nullptr, nullptr,
		budget_shm_name.empty() ? nullptr : budget_shm_name.c_str(),
		early_exit_shm_name.empty() ? nullptr : early_exit_shm_name.c_str(),
		k_ref,
		thread_count_shm_name.empty() ? nullptr : thread_count_shm_name.c_str(),
		latency_shm_name.empty() ? nullptr : latency_shm_name.c_str());
