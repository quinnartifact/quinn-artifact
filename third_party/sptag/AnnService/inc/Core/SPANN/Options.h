...
namespace SPTAG {
    namespace SPANN {
        ...
        public:
            ...
            std::string m_budgetShmName;      // QUINN: shared memory name for per-query budgets
            std::string m_earlyExitShm;       // QUINN: shared memory for early exit
            int m_earlyExitKRef;              // QUINN: k-th reference for early exit distance
            std::string m_threadCountShmName; // QUINN: SHM for dynamic thread count control
            std::string m_latencyShmName;     // QUINN: SHM for per-query latency timestamps
            int m_qpsLimit;
            int m_resultNum;
...
#undef DefineSSDParameter
                ...
                // QUINN: Initialize budget shm name to empty
                m_budgetShmName = "";
                m_earlyExitShm = "";
                m_earlyExitKRef = 1;
                m_threadCountShmName = "";
                m_latencyShmName = "";
            }
...
#undef DefineBuildHeadParameter
                    ...
                    // QUINN: Handle BudgetShmName parameter
                    if (Helper::StrUtils::StrEqualIgnoreCase(p_param, "BudgetShmName")) {
                        LOG(Helper::LogLevel::LL_Info, "Setting BudgetShmName with value %s\n", p_value);
                        m_budgetShmName = p_value;
                    }
                    else if (Helper::StrUtils::StrEqualIgnoreCase(p_param, "EarlyExitShm")) {
                        LOG(Helper::LogLevel::LL_Info, "Setting EarlyExitShm with value %s\n", p_value);
                        m_earlyExitShm = p_value;
                    }
                    else if (Helper::StrUtils::StrEqualIgnoreCase(p_param, "EarlyExitKRef")) {
                        LOG(Helper::LogLevel::LL_Info, "Setting EarlyExitKRef with value %s\n", p_value);
                        int tmp;
                        if (Helper::Convert::ConvertStringTo<int>(p_value, tmp)) {
                            m_earlyExitKRef = tmp;
                        }
                    }
                    else if (Helper::StrUtils::StrEqualIgnoreCase(p_param, "ThreadCountShmName")) {
                        LOG(Helper::LogLevel::LL_Info, "Setting ThreadCountShmName with value %s\n", p_value);
                        m_threadCountShmName = p_value;
                    }
                    else if (Helper::StrUtils::StrEqualIgnoreCase(p_param, "LatencyShmName")) {
                        LOG(Helper::LogLevel::LL_Info, "Setting LatencyShmName with value %s\n", p_value);
                        m_latencyShmName = p_value;
                    }
                    else {
#define DefineSSDParameter(VarName, VarType, DefaultValue, RepresentStr) \
    if (Helper::StrUtils::StrEqualIgnoreCase(p_param, RepresentStr)) \
