                                                 ...
                                                 const EarlyExitContext& early_exit)
{
    cached_beam_search(query1, k_search, l_search, indices, distances, beam_width, use_filter, filter_label,
                       std::numeric_limits<uint32_t>::max(), use_reorder_data, stats, early_exit);
}
...
void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                 ...
                                                 const EarlyExitContext& early_exit)
{
    // Early return when budget is zero — skip all work including PQ preprocessing
    if (l_search == 0)
    {
    ...
    // Set of SPANN top-K node IDs used for deprioritization during beam search.
    // Built from SHM when early_exit.deprioritize_spann is true.
    std::unordered_set<uint32_t> spann_id_set;

    // QUINN: inject SPANN topk as additional starting points.
    // PQ distances are computed in-memory (no disk IO).
    // Beam search then expands the most promising seeds first.
    //
    // Two modes controlled by early_exit.seed_indices:
    //   empty  → sequential: inject first min(topk_k, L-1) IDs (closest-first)
    //   non-empty → spread: pick the specific 0-based positions listed in seed_indices
    //
    // If early_exit.wait_for_spann is true, spin-yield until SPANN has written
    // its results (value transitions from FLT_MAX to actual distance).
    if (early_exit.wait_for_spann && early_exit.shm != nullptr && early_exit.shm->topk_k() > 0)
    {
        uint32_t _spin_count = 0;
        while (early_exit.shm->get(early_exit.query_id) == FLT_MAX)
        {
            _mm_pause();
            if (++_spin_count >= 64)
            {
    ...
    if (early_exit.shm != nullptr && early_exit.shm->topk_k() > 0 &&
        early_exit.shm->get(early_exit.query_id) != FLT_MAX)
    {
        const uint32_t topk_k = early_exit.shm->topk_k();
        std::vector<uint32_t> raw_ids(topk_k);
        early_exit.shm->get_topk(early_exit.query_id, raw_ids.data(), topk_k);

        std::vector<uint32_t> seed_ids;
        if (!early_exit.seed_indices.empty())
        {
            // Spread mode: pick only the positions specified by seed_indices.
            // Positions that are out-of-range or point to UINT32_MAX are skipped.
            for (uint32_t pos : early_exit.seed_indices)
            {
                if (pos < topk_k)
                {
                    uint32_t id = raw_ids[pos];
                    if (id != UINT32_MAX && visited.insert(id).second)
                        seed_ids.push_back(id);
                }
            }
        }
        else if (early_exit.seed_k > 0)
        {
            // Sequential mode: take the first seed_k IDs (independent of topk_k,
            // which only controls how many IDs SPANN writes to SHM).
            uint32_t n = std::min(early_exit.seed_k, topk_k);
            for (uint32_t j = 0; j < n; j++)
            {
                uint32_t id = raw_ids[j];
                if (id != UINT32_MAX && visited.insert(id).second)
                    seed_ids.push_back(id);
            }
        }
        // seed_k == 0 and no seed_indices: skip seed injection entirely.

        if (!seed_ids.empty())
        {
            // Cap at l_search-1: medoid already occupies 1 slot, so injecting at most
            // l_search-1 seeds fills retset to exactly capacity with no eviction.
            size_t n_inject = std::min(seed_ids.size(), l_search > 1 ? (size_t)(l_search - 1) : (size_t)0);
            std::vector<float> seed_dists(n_inject);
            compute_dists(seed_ids.data(), n_inject, seed_dists.data());
            for (size_t j = 0; j < n_inject; j++)
                retset.insert(Neighbor(seed_ids[j], seed_dists[j]));
        }

        // Build deprioritization set: beam search will defer these nodes to the
        // back of each iteration's frontier so that non-SPANN candidates expand first.
        if (early_exit.deprioritize_spann)
        {
            for (uint32_t id : raw_ids)
                if (id != UINT32_MAX)
                    spann_id_set.insert(id);
        }
    }
    ...
    uint32_t patience_count = 0;
    std::vector<Neighbor> beam_candidates;
    beam_candidates.reserve(beam_width);
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        // find new beam
        // Collect up to beam_width candidates from retset in distance order.
        // deprioritize_spann: if ALL beam_width candidates are SPANN nodes, replace
        // half of them (the worst half) with the best available non-SPANN nodes.
        while (retset.has_unexpanded_node() && beam_candidates.size() < beam_width)
        {
            auto nbr = retset.closest_unexpanded();
            if (early_exit.hop_trace != nullptr)
                early_exit.hop_trace->push_back({hops, nbr.id, nbr.distance});
            if (this->_count_visited_nodes)
                reinterpret_cast<std::atomic<uint32_t> &>(this->_node_visit_counter[nbr.id].second).fetch_add(1);
            beam_candidates.push_back(nbr);
        }

        if (early_exit.deprioritize_spann && !spann_id_set.empty() &&
            beam_candidates.size() == beam_width)
        {
            bool all_spann = true;
            for (const auto &c : beam_candidates)
                if (!spann_id_set.count(c.id)) { all_spann = false; break; }

            if (all_spann)
            {
                // Keep the best ceil(W/2) SPANN nodes; replace the rest with
                // up to floor(W/2) non-SPANN nodes from the remaining retset.
                uint32_t n_nonspann = beam_width / 2;
                uint32_t n_spann    = beam_width - n_nonspann;  // ceil(W/2)
                beam_candidates.resize(n_spann);  // drop worst SPANN nodes (already expanded)

                uint32_t found = 0;
                while (retset.has_unexpanded_node() && found < n_nonspann)
                {
                    auto nbr = retset.closest_unexpanded();
                    if (early_exit.hop_trace != nullptr)
                        early_exit.hop_trace->push_back({hops, nbr.id, nbr.distance});
                    if (this->_count_visited_nodes)
                        reinterpret_cast<std::atomic<uint32_t> &>(this->_node_visit_counter[nbr.id].second).fetch_add(1);
                    if (!spann_id_set.count(nbr.id))
                    {
                        beam_candidates.push_back(nbr);
                        found++;
                    }
                    // SPANN nodes encountered during this scan: marked expanded, not fetched.
                }
            }
        }

        for (const auto &nbr : beam_candidates)
        {
            if (frontier.size() + cached_nhoods.size() >= beam_width)
                break;
        ...
        {
            T *node_fp_coords_copy = _coord_cache.at(cached_nhood.first);
            float cur_expanded_dist;
            if (!_use_disk_index_pq)
            {
        ...
        // Incumbent-gated frontier bound early termination
        if (early_exit.enabled) {
            float d_frontier_best = retset.peek_unexpanded_distance();

            // τ_disk = tau_k_disk-th distance in current DiskANN results
            // float tau_disk = (retset.size() >= early_exit.tau_k_disk)
            //     ? retset[early_exit.tau_k_disk - 1].distance
            //     : std::numeric_limits<float>::max();

            // τ_spann from shared memory (concurrent SPANN search)
            float tau_spann = std::numeric_limits<float>::max();
            if (early_exit.shm != nullptr) {
                tau_spann = early_exit.shm->get(early_exit.query_id);
            }

            // float tau = std::min(tau_spann, tau_disk);

            if (d_frontier_best >= tau_spann * (1.0f + early_exit.eps_stop)) {
                patience_count++;
                if (patience_count >= early_exit.patience) {
                    break;
                }
            } else {
                patience_count = 0;
