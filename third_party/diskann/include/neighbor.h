...
class NeighborPriorityQueue
{
  public:
    ...
    // Peek at the distance of the closest unexpanded node without expanding it.
    float peek_unexpanded_distance() const
    {
        if (_cur < _size)
            return _data[_cur].distance;
        return std::numeric_limits<float>::max();
    }

    // Peek at the ID of the closest unexpanded node without expanding it.
    uint32_t peek_unexpanded_id() const
    {
        if (_cur < _size)
            return _data[_cur].id;
