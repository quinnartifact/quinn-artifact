...
namespace diskann
{
    struct HopTraceEntry {
        uint32_t hop;
        uint32_t node_id;
        float distance;  // distance of expanded node to query
    };

    struct EarlyExitContext {
        bool enabled = false;
        float eps_stop = 0.05f;
        uint32_t tau_k_spann = 100;   // use SPANN's tau_k_spann-th distance as τ_spann
        uint32_t tau_k_disk = 100;    // use DiskANN's tau_k_disk-th distance as τ_disk
        uint32_t patience = 1;        // consecutive rounds frontier bound must hold before terminating
        quinn::EarlyExitShmAccessor* shm = nullptr;
        uint64_t query_id = 0;
        std::vector<HopTraceEntry>* hop_trace = nullptr;  // if non-null, record expanded nodes
        // QUINN: if non-empty, pick these 0-based positions from SHM topk as seeds
        // (e.g. {0,25,50,75} spreads 4 seeds across SPANN top-100).
        // When empty, fall back to sequential top-N injection (original behaviour).
        std::vector<uint32_t> seed_indices;
        // Number of SPANN top-K IDs to inject as DiskANN retset seeds (sequential mode).
        // 0 = no seed injection. Ignored when seed_indices is non-empty (spread mode).
        uint32_t seed_k = 0;
        // If true and topk_k > 0, spin-wait (yield) until SPANN has written its
        // top-K IDs to SHM before starting beam search.  Guarantees seeds are
        // always injected; DiskANN never falls back to medoid-only start.
        bool wait_for_spann = false;
        // If true, nodes already in SPANN's top-K results are moved to the back
        // of the beam frontier each iteration: non-SPANN candidates expand first,
        // SPANN-covered nodes fill remaining beam slots only if needed.
        // Requires topk_k > 0 so that SPANN IDs are available in SHM.
